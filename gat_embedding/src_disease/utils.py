...
def get_disease_mapping(graph):
    disease_mapping = {}  # Mapping of node_id to disease
    for node_id, data in graph.graph_nx.nodes(data=True):
        disease = data['disease']
        disease_mapping[node_id] = disease  # Store the mapping
    return disease_mapping  # Return the disease mapping
...
def save_graph_to_neo4j(graph):
    ...
    for node_id, data in graph.graph_nx.nodes(data=True):
        disease = data['disease']
        node = Node("Pathway", disease=disease, name=data['name'], weight=data['weight'], significance=data['significance'])
        nodes[node_id] = node
...
def create_network(path_disease_csv, kge):
    graph = network.Network(path_disease_csv, kge)
    return graph
...
def save_disease_to_csv(graph, save_dir):
    assert os.path.isdir(save_dir), 'Directory does not exist!'
    disease_data = {'disease': [node['disease'] for node in graph.graph_nx.nodes.values()]}
    df = pd.DataFrame(disease_data)
    csv_path = os.path.join(save_dir, 'disease_nodes.csv')
    df.to_csv(csv_path, index=False)

def create_graphs(save=True, data_dir='gat/data/emb'):
    graph_train = create_network('gat/data/disease_p_value_results_train.csv', 'emb_train')
    graph_test = create_network('gat/data/disease_p_value_results_test.csv', 'emb_test')
